    ...
    <div className="bg-gradient-to-b from-neutral-900 via-neutral-900 to-neutral-950 border border-neutral-800 rounded-2xl p-5 sm:p-7 text-white shadow-xl relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-neutral-800/80">
        ...
        <div className="flex items-center gap-2 text-xs font-mono">
          <div className="px-3 py-1.5 rounded-lg bg-neutral-800/80 border border-neutral-700/60 text-neutral-300 flex items-center gap-2">
            ...
            return (
              <div
                ...
                className={`relative group p-4 rounded-xl transition-all duration-200 cursor-pointer border ${
                  isHovered
                    ? "bg-neutral-800/90 border-neutral-600 shadow-lg scale-[1.02]"
                    : "bg-neutral-800/40 border-neutral-800 hover:border-neutral-700 hover:bg-neutral-800/60"
                }`}
              >
                ...
                {index < nodes.length - 1 && (
                  <div className="hidden md:flex absolute -right-3 top-1/2 -translate-y-1/2 z-20 items-center justify-center">
                    ...
                    <ArrowRight className="w-3 h-3 text-neutral-500 group-hover:text-emerald-400 transition-colors -ml-1" />
                  </div>
                )}
                ...
                <div className="flex flex-col h-full justify-between space-y-3">
                  <div>
                    <div className="flex items-center justify-between gap-1 mb-2">
                      <div className="w-8 h-8 rounded-lg bg-neutral-700/50 border border-neutral-600/50 flex items-center justify-center text-white group-hover:border-emerald-500/40 group-hover:bg-emerald-500/10 transition-colors">
                        <Icon className="w-4 h-4 text-emerald-400" />
                      </div>
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-neutral-900 border border-neutral-700 text-neutral-300">
                        {node.badge}
                      </span>
                    </div>
                  ...
                  <div className="pt-2 border-t border-neutral-800 text-[10px] font-mono text-neutral-300 flex items-center justify-between">
                    <span className="text-emerald-400 flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3" />
                      {node.status}
                    </span>
                    <span className="text-neutral-400 text-right truncate pl-1">
                      {node.metrics}
                    </span>
                  </div>
                </div>
      ...
      {activeNode ? (
        <div className="relative z-10 mt-2 p-3.5 rounded-xl bg-neutral-800/80 border border-neutral-700/80 text-xs flex items-center justify-between gap-4 animate-in fade-in duration-150">
          ...
          <span className="text-[10px] font-mono text-neutral-400 shrink-0 bg-neutral-900 px-2 py-1 rounded">
            Node: {activeNode}
          </span>
        </div>
      ) : (
        <div className="relative z-10 mt-2 p-3 rounded-xl bg-neutral-900/60 border border-neutral-800/80 text-[11px] text-neutral-400 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span>Hover over any pipeline node to inspect real-time component responsibilities and throughput specs.</span>
          </div>
          <span className="font-mono text-[10px] text-neutral-500 hidden sm:inline-block">FIFO Mutex Locked</span>
        </div>
      )}
    </div>
  );
