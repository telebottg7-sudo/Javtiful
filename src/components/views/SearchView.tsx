...
export const SearchView: React.FC<SearchViewProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="space-y-3">
        <form onSubmit={handleSearchSubmit} className="relative flex items-center">
          ...
          <input
            ...
            className="w-full pl-12 pr-28 py-3.5 bg-white border border-neutral-300 rounded-xl text-neutral-900 placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:border-transparent text-sm shadow-xs transition-all"
          />
          <div className="absolute right-3 flex items-center gap-2">
            ...
            <button
              ...
              type="submit"
              disabled={loading}
              className="px-3.5 py-1.5 bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-medium rounded-lg shadow-xs transition-colors flex items-center gap-1.5"
            >
              {loading ? (
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 text-xs text-neutral-500">
          ...
          {quickSuggestions.map((item, idx) => (
            <button
              ...
              className="px-2.5 py-1 rounded-md bg-neutral-100 hover:bg-neutral-200 text-neutral-700 font-medium transition-colors shrink-0"
            >
              {item.label}
            </button>
          ))}
      ...
      <div className="flex items-center justify-between border-b border-neutral-200 overflow-x-auto">
        <div className="flex items-center gap-1">
          ...
          >
            ...
            All Results
            {searchResponse && (
              <span className="ml-1 px-1.5 py-0.5 rounded-full bg-neutral-100 text-neutral-600 text-[10px] font-mono">
      ...
      {/* Direct Match Banner (if query exactly matched a canonical code or entity) */}
          {searchResponse?.directMatch && (
            <div className="p-4 rounded-xl bg-neutral-900 text-white border border-neutral-800 shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-neutral-800 text-amber-400 shrink-0">
                  <Sparkles className="w-5 h-5" />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded bg-amber-400 text-neutral-950 font-bold text-[10px] tracking-wider uppercase">
              ...
              <div className="flex items-center gap-2 shrink-0">
                {searchResponse.directMatch.type === "video" || searchResponse.directMatch.type === "code" ? (
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-lg bg-white hover:bg-neutral-100 text-neutral-950 text-xs font-semibold flex items-center gap-1.5 transition-colors shadow-xs"
                  >
          ...
          {loading ? (
            ...
            <div className="p-12 text-center bg-white rounded-xl border border-neutral-200 text-neutral-500 space-y-3">
              ...
              {onNavigate && (
                <button
                  ...
                  className="mt-2 inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md bg-neutral-900 text-white hover:bg-neutral-800"
                >
                  ...
                  >
                    ...
                    {item.type === "video" ? (
                      <div className="relative aspect-16/10 bg-neutral-900 overflow-hidden">
                        ...
                        {item.code && (
                          <span className="absolute top-2 left-2 px-2 py-0.5 rounded bg-black/85 text-white font-mono text-[11px] font-bold">
                            {item.code}
                          </span>
                        )}
                        {item.duration && (
                          <span className="absolute bottom-2 right-2 px-1.5 py-0.5 rounded bg-black/85 text-white font-mono text-[10px] flex items-center gap-1">
                      ...
                      <div className="p-4 bg-neutral-100 border-b border-neutral-200 flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <span className="px-2 py-0.5 rounded-full bg-neutral-200 text-neutral-800 text-[10px] font-mono font-bold uppercase tracking-wider">
                            Registry Code
                          </span>
                          <h4 className="text-sm font-mono font-bold text-neutral-900 truncate mt-0.5">
                      ...
                      <div className="pt-2 border-t border-neutral-100">
                        {item.type === "video" ? (
                          <button
                            ...
                            className="w-full py-1.5 px-3 rounded-lg bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-medium flex items-center justify-center gap-1.5 transition-colors shadow-2xs"
                          >
                          ...
                          <button
                            ...
                            onClick={() => openHarvesterModal(item.postUrl || item.code || "")}
                            className="w-full py-1.5 px-3 rounded-lg bg-neutral-100 hover:bg-neutral-200 text-neutral-800 text-xs font-medium flex items-center justify-center gap-1.5 transition-colors"
                          >
                            <Download className="w-3.5 h-3.5" />
