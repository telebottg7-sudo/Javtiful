    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-5 sm:p-6 shadow-xs space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-neutral-300 hover:bg-neutral-50 text-xs font-medium text-neutral-700 transition-colors"
            >
            ...
            {onNavigate && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-neutral-900 text-white hover:bg-neutral-800 text-xs font-medium transition-colors"
              >
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 pt-2 border-t border-neutral-100">
          ...
          <div className="flex items-center gap-3 text-xs text-neutral-500 whitespace-nowrap self-start sm:self-auto">
            ...
            {(searchQuery || actressFilter || studioFilter) && (
              <button
                ...
                className="text-xs text-neutral-700 underline font-medium hover:text-neutral-900"
              >
                Reset Filters
              </button>
            )}
        ...
        {(actressFilter || studioFilter) && (
          <div className="pt-2 flex items-center gap-2 text-xs">
            ...
            {actressFilter && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-neutral-100 text-neutral-800 text-xs font-medium">
                <Users className="w-3 h-3 text-neutral-500" />
                Actress: {actressFilter}
                <button onClick={() => setActressFilter("")} className="hover:text-neutral-900">
                  <X className="w-3 h-3" />
                </button>
              </span>
            )}
            {studioFilter && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-neutral-100 text-neutral-800 text-xs font-medium">
                ...
                Studio: {studioFilter}
                <button onClick={() => setStudioFilter("")} className="hover:text-neutral-900">
                  <X className="w-3 h-3" />
                </button>
              </span>
            )}
      ...
      {loading ? (
        <div className="bg-white border border-neutral-200 rounded-xl p-12 text-center space-y-3">
          ...
          <p className="text-xs text-neutral-500">Loading master video index...</p>
        </div>
      ) : videos.length === 0 ? (
        <div className="bg-white border border-neutral-200 rounded-xl p-12 text-center space-y-4">
          ...
          <div className="space-y-1">
            <h3 className="text-sm font-semibold text-neutral-900">No Videos In Database</h3>
            ...
            {videos.map((vid, idx) => (
              ...
              >
                ...
                <div className="relative aspect-16/10 bg-neutral-100 overflow-hidden">
                  ...
                  {vid.code && (
                    <div className="absolute top-2 left-2">
                      <span className="px-2 py-0.5 rounded-md bg-neutral-900/90 backdrop-blur-xs text-white text-[11px] font-mono font-semibold shadow-xs">
                        {vid.code}
                      </span>
                    </div>
                  )}
                ...
                <div className="p-3.5 space-y-2 flex-1 flex flex-col justify-between">
                  <h3 className="text-xs font-semibold text-neutral-900 line-clamp-2 leading-tight group-hover:text-neutral-700">
                    {vid.title}
                  </h3>
                  ...
                  <div className="space-y-1.5 text-[11px] text-neutral-600">
                    {vid.actressName && (
                      <div
                        ...
                        className="flex items-center gap-1 text-neutral-700 hover:text-neutral-900 truncate cursor-pointer"
                      >
          ...
          <div className="bg-white rounded-2xl border border-neutral-200 shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="p-5 border-b border-neutral-200 flex items-center justify-between gap-4 bg-neutral-50 shrink-0">
              <div className="flex items-center gap-2 min-w-0">
                {selectedVideo.code && (
                  <span className="px-2.5 py-1 rounded-md bg-neutral-900 text-white font-mono text-xs font-semibold">
                    {selectedVideo.code}
                  </span>
                )}
            ...
            <div className="p-5 overflow-y-auto space-y-4">
              ...
              {selectedVideo.thumbnail && (
                <div className="rounded-xl overflow-hidden border border-neutral-200 bg-neutral-100 max-h-72 flex items-center justify-center">
                  <img
              ...
              <div className="rounded-xl border border-neutral-200 overflow-hidden divide-y divide-neutral-100 text-xs">
                <div className="p-3 bg-neutral-50 flex items-center justify-between">
                  <span className="text-neutral-500 font-medium">Canonical Code</span>
                  <span className="font-mono font-semibold text-neutral-900">{selectedVideo.code}</span>
                </div>
                <div className="p-3 flex items-center justify-between">
                  <span className="text-neutral-500 font-medium">Actress</span>
                  <span className="font-medium text-neutral-900">
                    {selectedVideo.actressName ? `${selectedVideo.actressName} (${selectedVideo.actressSlug})` : "None"}
                  </span>
                </div>
                <div className="p-3 bg-neutral-50 flex items-center justify-between">
                  <span className="text-neutral-500 font-medium">Studio / Maker</span>
                  <span className="font-medium text-neutral-900">
                    {selectedVideo.studioName ? `${selectedVideo.studioName} (${selectedVideo.studioSlug})` : "None"}
                  </span>
                </div>
                <div className="p-3 flex items-center justify-between">
                  <span className="text-neutral-500 font-medium">Release Date</span>
                  <span className="font-mono text-neutral-900">{selectedVideo.releaseDate || "N/A"}</span>
                </div>
                <div className="p-3 bg-neutral-50 flex items-center justify-between">
                  <span className="text-neutral-500 font-medium">Duration</span>
                  <span className="font-mono text-neutral-900">{selectedVideo.duration || "N/A"}</span>
                </div>
                <div className="p-3 flex items-center justify-between">
                  <span className="text-neutral-500 font-medium">Post / Stream URL</span>
                  <a
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={copyJson}
                    className="inline-flex items-center gap-1 px-2 py-0.5 text-[11px] rounded border border-neutral-300 hover:bg-neutral-50 text-neutral-700 transition-colors"
                  >
                    {copied ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />}
                    <span>{copied ? "Copied" : "Copy"}</span>
                  </button>
                </div>
                <div className="p-3 bg-neutral-900 text-emerald-400 rounded-xl font-mono text-xs overflow-x-auto max-h-40">
            ...
            {/* Modal Footer */}
            <div className="p-4 border-t border-neutral-200 bg-neutral-50 flex items-center justify-between text-xs gap-2">
              <div className="flex items-center gap-2">
                <a
                  href={selectedVideo.postUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-neutral-300 bg-white hover:bg-neutral-50 text-neutral-700 font-medium"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-neutral-900 hover:bg-neutral-800 text-white font-medium shadow-xs"
                >
              ...
              <button
                onClick={() => setSelectedVideo(null)}
                className="px-4 py-1.5 rounded-lg border border-neutral-300 bg-white text-neutral-700 hover:bg-neutral-50 font-medium transition-colors"
              >
                Close
              </button>
            </div>
